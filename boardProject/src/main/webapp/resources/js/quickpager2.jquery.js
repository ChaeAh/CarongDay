...
(function($) {
	...
	$.fn.quickPager = function(options) {
		...
		if(pageCounter > 1) {
			...
			var view_start =  Math.floor((options.currentPage - 1) / options.naviSize) * options.naviSize + 1;
			var view_end =  Math.ceil(options.currentPage / options.naviSize) * options.naviSize;

			view_end = (view_end > pageCounter ? pageCounter : view_end);

			if(view_start == options.currentPage && view_end == options.currentPage) {
				$('.prev-'+(view_end-1)).show();
			} else {
				for (i=view_start; i<=view_end; i++) {
					$('.pageNav'+i).show();
				}

				$('.prev-'+(view_start-1)).show();
				$('.next-'+(view_end+1)).show();
			}
			...
			$(".pageNav a").live("click", function() {			
				...
				$("ul.pageNav").find("a[rel='"+clickedLink+"']").parent("li[class^='pageNav']").addClass("currentPage");
